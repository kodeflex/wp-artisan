...
function initialize(path, config, options) {
  ...
  wpCli.call("core download", args, (err, resp) => {
    ...
    for (let i = 0; i < deleteList.length; i++) {
      deleteList[i] = deleteList[i].replace(
        "<siteDir>",
        `${workingDir}/${env.defaults.installDir}`
      );
    }
...
module.exports = {
  isInitialized(path) {
    const workingDir = Utils.getWorkingDir(path);
    const isConfigExists = Utils.isFileExists(
  ...
  initialize(path, options = null) {
    ...
    inquirer.prompt(promptSchema.config).then(config => {
      // update configs.
      const serverHost = config.server.host
        ? config.server.host
        : env.defaults.server.host;
      const serverPort = config.server.port
        ? config.server.port
        : env.defaults.server.port;
  ...
  startServer(options) {
    ...
    const config = JSON.parse(
      Utils.readFileSync(`${workingDir}/${env.defaults.config}`, "utf-8")
    );
